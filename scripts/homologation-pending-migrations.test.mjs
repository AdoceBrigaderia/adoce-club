...
test("plano real contém as 22 migrations em ordem e aguarda somente o dry-run", async () => {
  ...
  assert.equal(report.pending_migrations.at(-1).file, "20260728225000_real_configurable_product_catalog.sql");
});
...
test("rejeita aplicação parcial quando um arquivo planejado não existe", async () => {
  const { plan, files, remoteNames } = await loadFixture();
  const withoutRealCatalog = files.filter((file) => file !== "20260728225000_real_configurable_product_catalog.sql");
  const report = inspectPendingMigrationPlan(plan, withoutRealCatalog, remoteNames);
  assert.equal(report.structural_passed, false);
  assert.deepEqual(report.missing_planned_files, ["20260728225000_real_configurable_product_catalog.sql"]);
});
