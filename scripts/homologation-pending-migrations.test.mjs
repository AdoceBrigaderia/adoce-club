...
test("plano real contém as 19 migrations em ordem e aguarda somente o dry-run", async () => {
  ...
  assert.equal(report.pending_count, 19);
  ...
  assert.equal(report.pending_migrations.at(-1).file, "20260728221500_configurable_product_constraints.sql");
});
...
test("rejeita aplicação parcial quando um arquivo planejado não existe", async () => {
  const { plan, files, remoteNames } = await loadFixture();
  const withoutConstraints = files.filter((file) => file !== "20260728221500_configurable_product_constraints.sql");
  const report = inspectPendingMigrationPlan(plan, withoutConstraints, remoteNames);
  assert.equal(report.structural_passed, false);
  assert.deepEqual(report.missing_planned_files, ["20260728221500_configurable_product_constraints.sql"]);
});
