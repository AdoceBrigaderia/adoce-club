...
// diferença de latência denuncia quais celulares têm cadastro.
...
export default async (request: Request) => {
  ...
  const [phoneLimit, ipLimit] = await Promise.all([
    consumeRateLimit(admin, "customer_login:phone", phoneHash, 900, 8),
    consumeRateLimit(admin, "customer_login:ip", ipHash, 900, 30),
  ]);
  ...
  const eligible = Boolean(
    profile?.id && profile.active && profile.account_status === "active" && profile.auth_upgraded_at,
  );
