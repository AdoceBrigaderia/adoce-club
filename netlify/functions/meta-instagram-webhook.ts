...
export default async (request: Request) => {
  ...
  if (request.method === "GET") {
    ...
    return metaTextResponse("Webhook n?o autorizado.", 403);
  }

  if (request.method !== "POST") return metaTextResponse("M?todo n?o permitido.", 405);
  ...
  if (!(await validMetaSignature(rawBody, signature, appSecret)))
    return metaTextResponse("Assinatura inv?lida.", 401);

  let payload: unknown;
  try {
    payload = JSON.parse(rawBody);
  } catch {
    return metaTextResponse("JSON inv?lido.", 400);
  }
  ...
  if (!expectedAccountId || events.some((event) => event.accountId !== expectedAccountId))
    return metaTextResponse("Conta do evento n?o autorizada.", 403);

  const supabaseUrl = metaEnv("SUPABASE_URL") || "";
  const secretKey = metaEnv("SUPABASE_SECRET_KEY") || metaEnv("SUPABASE_SERVICE_ROLE_KEY") || "";
  if (!supabaseUrl || !secretKey) return metaTextResponse("Servidor n?o configurado.", 503);
  ...
  if (accountError || !account) {
    ...
    return metaTextResponse("Falha tempor?ria ao registrar evento.", 503);
  }
