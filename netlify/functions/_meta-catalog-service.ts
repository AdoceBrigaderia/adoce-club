...
export async function authorizeMetaAdmin(request: Request, requireMetaConfiguration = true): Promise<MetaServiceContext | Response> {
  ...
  if (!config.supabaseUrl || !config.publishableKey || !config.secretKey)
    return json({ error: "Ação administrativa não configurada." }, 503);
  ...
  if (!staff?.active || !["owner", "manager"].includes(staff.role))
    return json({ error: "Somente proprietários e gerentes podem sincronizar o catálogo." }, 403);
  ...
  if (requireMetaConfiguration && !metaConfigured)
    return json({ error: "Integração com a Meta aguardando configuração no servidor.", code: "META_NOT_CONFIGURED" }, 503);
  ...
  try {
    ...
    if (!prepared) {
      ...
      await history(context, {
        ...
        status: "skipped", response_summary: "Produto não marcado para o WhatsApp e nunca enviado.", attempt: 1,
...
export async function metaStatus(context: MetaServiceContext) {
  ...
  if (context.client) {
    try {
      ...
      connection = { connected: false, error: validationError instanceof Error ? validationError.message : "Não foi possível validar a conexão." };
    }
  } else {
    connection = { connected: false, error: "Credenciais da Meta ainda não configuradas no servidor." };
  }
  const rows = products || [];
  return {
