...
export function formatMetaPrice(value: number) {
  if (!Number.isFinite(value) || value <= 0) throw new Error("O produto precisa ter preço maior que zero para a Meta.");
...
export async function prepareMetaItem(product: CatalogProductRow, productBaseUrl: string): Promise<PreparedMetaItem | null> {
  ...
  if (product.base_price === null) throw new Error("Defina o preço antes de exibir este produto no WhatsApp.");
  if (!isPublicHttpsUrl(product.image_url)) throw new Error("Defina uma imagem principal com URL pública HTTPS.");
  if (!isPublicHttpsUrl(productBaseUrl)) throw new Error("Configure uma URL pública HTTPS para os produtos.");
...
export class MetaCatalogClient {
  ...
  constructor(
    ...
    if (!/^\d+\.\d+$/.test(version)) throw new Error("META_GRAPH_API_VERSION inválida.");
  ...
  private async request(path: string, init: RequestInit, maxAttempts = 3) {
    ...
    throw lastError || new MetaCatalogError("Falha desconhecida na integração com a Meta.");
  }
  ...
  async submitItem(item: PreparedMetaItem) {
    ...
    if (!handles[0]) throw new MetaCatalogError("A Meta aceitou a chamada, mas não devolveu o identificador do lote.", 502, safeSummary(result.payload));
