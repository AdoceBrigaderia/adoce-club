...
const maskPhone = (value: string | null) => {
  ...
  return `(${digits.slice(0, 2)}) ${digits
    .slice(2, -4)
    .replace(/.(?=.{3})/g, "•")}‑${digits.slice(-4)}`;
};
...
export default function PasskeyClientGateway() {
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    setMessage("");
    try {
      const data = await clientBffRpc<CustomerWorkspace>(
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Não foi possível abrir seu Clube Adoce.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const refreshSession = useCallback(async () => {
    ...
    setSession(next);
    if (window.location.hash.startsWith("#entrar") && continueAfterLogin())
      return;
    await loadWorkspace();
  ...
  const passwordLogin = async (event: React.FormEvent) => {
    ...
    setMessage("");
    try {
      const next = await bffPasswordLogin({
        phone,
        password,
        surface: "client",
        remember,
      ...
      if (next.mustChangePassword) {
        ...
        setMessage(
          "Sua senha precisa ser atualizada antes do acesso. Use a recuperação pelo WhatsApp.",
        );
        return;
      }
      setSession(next);
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Não foi possível entrar no Clube.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const passkeyLogin = async () => {
    ...
    try {
      ...
      setMessage(
        text.toLocaleLowerCase("pt-BR").includes("cancel")
          ? "A biometria foi cancelada."
          : text,
      );
    } finally {
      setBusy(false);
  ...
  const issueQr = async () => {
    ...
    setMessage("");
    try {
      const raw = await clientBffRpc<IssuedQr | IssuedQr[]>(
        "issue_customer_qr",
      );
      const issued = Array.isArray(raw) ? raw[0] : raw;
      if (!issued?.token)
        throw new Error("Não foi possível gerar seu QR agora.");
      const target = new URL("/", window.location.origin);
      target.searchParams.set("cartao", issued.token);
      target.hash = "cartao";
      setQrImage(
        await QRCode.toDataURL(target.toString(), {
          width: 640,
          margin: 2,
  ...
  const progress = Math.max(
    0,
    Math.min(14, Number(workspace?.loyalty.current_progress || 0)),
  );
      ...
      <main className="passkey-client-gateway login">
        <header className="passkey-client-header">
          <a href="/" aria-label="Voltar para Adoce Brigaderia">
            ...
            <span>
              ...
              <small>Seu cartão digital</small>
            </span>
          </a>
        ...
        <section className="passkey-client-login-shell">
          <div className="passkey-client-login-copy">
            ...
            <p>
              Entre com a biometria do celular, o PIN do aparelho ou sua senha.
              Sua digital e seu Face ID nunca são enviados para a Adoce.
            </p>
          </div>
          ...
          <div className="passkey-client-login-card">
            {passkeyReady ? (
              <button
                className="passkey-client-biometric"
                type="button"
              ...
              >
                ...
                <span>
                  ...
                  <small>Digital, Face ID, PIN ou chave de acesso</small>
                </span>
              </button>
            ) : (
              <p className="passkey-client-unsupported">
                <Smartphone /> Este navegador não oferece chave de acesso. Use
                sua senha.
              </p>
            )}

            <div className="passkey-client-divider">
              <span>ou entre com sua senha</span>
            </div>
            ...
            <form onSubmit={passwordLogin}>
              ...
              <button className="passkey-client-submit" disabled={busy}>
            ...
            {message ? (
              <p className="passkey-client-message" role="status">
                {message}
              </p>
            ) : null}
          </div>
  ...
  return (
    <main className="passkey-client-gateway account">
      <header className="passkey-client-header">
        ...
        <div>
          {passkeyReady ? (
            <button type="button" onClick={() => setManagePasskeys(true)}>
      ...
      {loading && !workspace ? (
        <section className="passkey-client-loading">
          <RefreshCw className="spin" />
          <strong>Abrindo seu Clube Adoce…</strong>
        </section>
      ) : workspace ? (
        <>
          <section className="passkey-client-welcome">
            <div>
              ...
              <h1>Olá, {firstName}!</h1>
              <p>
                Seu cartão, seu QR e seus acessos seguros estão reunidos aqui.
              </p>
            </div>
            <span>
              <Gift />
              <strong>{workspace.loyalty.available_rewards}</strong>
              <small>fatia(s) grátis</small>
            </span>
          </section>
          ...
          <section className="passkey-client-grid">
            <article className="passkey-client-loyalty-card">
              ...
              <div className="passkey-client-member">
                ...
                <strong>{workspace.profile.member_code || "—"}</strong>
                <small>{workspace.profile.full_name}</small>
              </div>
              <div className="passkey-client-progress-title">
                <strong>{progress} de 14 carimbos</strong>
                <span>
                  {workspace.loyalty.available_rewards
                    ? "Fatia grátis disponível"
                    : `Faltam ${14 - progress}`}
                </span>
              </div>
              <div
                className="passkey-client-stamps"
                aria-label={`${progress} de 14 carimbos`}
              >
            ...
            <aside className="passkey-client-side">
              <article>
                ...
                <span>
                  ...
                  <p>
                    Encoste no NFC ou leia o QR da Adoce. Depois confirme com
                    biometria e seu cadastro aparece para a equipe.
                  </p>
                </span>
                <a href="/#check-in?loja=passare&caixa=principal">
                  Abrir check-in
                </a>
              </article>
              <article>
                ...
                <span>
                  ...
                  <p>
                    {maskPhone(workspace.profile.phone_e164)} ·{" "}
                    {workspace.profile.whatsapp_verified
                      ? "WhatsApp confirmado"
              ...
              <article>
                ...
                <span>
                  ...
                  <p>
                    A sessão não expõe seus tokens ao JavaScript. Cadastre
                    biometria para entrar ainda mais rápido.
                  </p>
                </span>
                {passkeyReady ? (
                  <button type="button" onClick={() => setManagePasskeys(true)}>
          ...
          <section className="passkey-client-history">
            ...
            {!workspace.recent_movements.length ? (
              <p>Nenhuma movimentação recente.</p>
            ) : (
              workspace.recent_movements.map((entry) => (
                <article key={entry.id}>
                  <span
                    className={entry.stamps_delta >= 0 ? "positive" : "negative"}
                  >
                    {entry.stamps_delta > 0 ? "+" : ""}
                    {entry.stamps_delta}
                  </span>
                  <div>
          ...
          {qrImage ? (
            <div
              className="passkey-client-qr-layer"
              role="dialog"
              aria-modal="true"
              aria-label="QR do Clube Adoce"
            >
              <button
                className="passkey-client-qr-backdrop"
                aria-label="Fechar"
                onClick={() => setQrImage("")}
              />
              <section>
                ...
                <div>
                  <img src={qrImage} alt="QR seguro do Clube Adoce" />
                  <img src="/site/logo.webp" alt="" />
                </div>
                <strong>
                  {qrExpiresAt ? (
                ...
                <button type="button" onClick={() => setQrImage("")}>
                  <Check /> Concluir
                </button>
              </section>
            </div>
          ...
          {managePasskeys ? (
            <PasskeyManager
              surface="client"
              onClose={() => setManagePasskeys(false)}
            />
          ) : null}
        </>
      ) : (
