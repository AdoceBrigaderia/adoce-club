  ...
  return <section className="commerce-finance-page print-scope">
    ...
    {summary ? <>
      ...
      <div className="commerce-report-grid">
        <section className="commerce-tool-card"><header><WalletCards /><div><small>Comparativo</small><h3>Por meio de pagamento</h3></div></header><table><thead><tr><th>Meio</th><th>Vendas</th><th>Bruto</th><th>Taxas</th><th>Líquido</th></tr></thead><tbody>{summary.by_method.map((row) => <tr key={row.code}><td data-label="Meio">{row.label}</td><td data-label="Vendas">{row.orders}</td><td data-label="Bruto">{money(row.gross)}</td><td data-label="Taxas">{money(row.fees)}</td><td data-label="Líquido" className="net">{money(row.net)}</td></tr>)}</tbody></table>{!summary.by_method.length ? <p>Nenhuma venda paga neste período.</p> : null}</section>
        <section className="commerce-tool-card"><header><CircleDollarSign /><div><small>Movimento</small><h3>Vendas por dia</h3></div></header><table><thead><tr><th>Dia</th><th>Vendas</th><th>Bruto</th><th>Líquido</th></tr></thead><tbody>{summary.by_day.map((row) => <tr key={row.sale_date}><td data-label="Dia">{new Date(`${row.sale_date}T12:00:00`).toLocaleDateString("pt-BR")}</td><td data-label="Vendas">{row.orders}</td><td data-label="Bruto">{money(row.gross)}</td><td data-label="Líquido" className="net">{money(row.net)}</td></tr>)}</tbody></table></section>
      </div>
    </> : null}
  </section>;
}
