import { ArrowDown, ArrowUp, ImagePlus, Video, Star, Trash2 } from "lucide-react";
...
export default function CommercialMediaAdmin({
  ...
  onAddReel: (url: string, caption: string) => Promise<void>;
  ...
  return <section className="commercial-media-admin">
    ...
    <div className="commercial-media-admin-add">
      ...
      <div className="commercial-media-admin-reel-form">
        ...
        <button type="button" disabled={busy || items.length >= 8 || reels >= 2 || !reelUrl.trim()} onClick={() => void onAddReel(reelUrl, reelCaption).then(() => { setReelUrl(""); setReelCaption(""); })}>Adicionar Reel</button>
      </div>
    </div>
  </section>;
}
