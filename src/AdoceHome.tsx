...
import { pickupWindowsForDay, pickupWindowHint, type PickupHour } from "./pickup-window";
...
export default function AdoceHome() {
  ...
  const [janelas, setJanelas] = useState<ReturnType<typeof pickupWindowsForDay>>([]);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setSabores(lista);
      setJanelas(pickupWindowsForDay((bh.data || []) as PickupHour[], weekday));
      setCarregando(false);
    ...
    <main className="adoce-home">
      ...
      <section className={`ah-status ${aberto ? "aberta" : "fechada"}`} role="status">
        ...
        <p className="ah-status-texto">
          {aberto
            ? `Retirada ${pickupWindowHint(janelas).replace("Hoje das ", "das ").replace(".", "")}, no Cantinho da Adoce`
              ...
              {visiveis.map((s) => (
                <li key={s.id} className="ah-fatia">
                  <div className="ah-foto">
                    <img src={s.imagem || "/site/hero-slice-real.webp"} alt={s.nome} loading="lazy" />
                  </div>
      ...
      <section className="ah-beth" aria-label="Sobre a Adoce">
        <img className="ah-beth-foto" src="/site/beth-fundadora-card.webp" alt="Beth, fundadora da Adoce Brigaderia, segurando uma torta de chocolate" loading="lazy" />
        <div>
