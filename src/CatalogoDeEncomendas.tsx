...
type CommercialRow = {
  ...
  lead_business_days: number | null;
...
function produtoDoBanco(row: CommercialRow): Produto {
  return {
    id: row.id,
    segmento: row.segment,
    nome: row.name,
    resumo: row.short_description || "",
    preco: Number(row.base_price || 0),
    sufixo: row.price_suffix || "",
    minimo: Number(row.minimum_quantity || 1),
    prazoDiasUteis: Number(row.lead_business_days || 1),
    fotoUrl: row.image_url,
    publicado: Boolean(row.published && row.active),
...
export default function CatalogoDeEncomendas({
  ...
  const [carregando, setCarregando] = useState(!produtosIniciais);

  useEffect(() => setAtivo(segmentoInicial), [segmentoInicial]);
    ...
    void (async () => {
      try {
        const { data, error } = await requireSupabase()
          ...
          .select("id,segment,name,short_description,base_price,price_suffix,minimum_quantity,lead_business_days,image_url,published,active,sort_order")
          .eq("active", true)
          .eq("published", true)
          .order("sort_order");
        if (!ativo || error) return;
        setProdutos(((data || []) as CommercialRow[]).map(produtoDoBanco));
      } finally {
  ...
  return (
    <main className="encomendas">
      ...
      {carregando ? <p className="enc-vazio">Carregando encomendas…</p> : null}
      {!lista.length ? (
