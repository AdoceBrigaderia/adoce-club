...
//
// A compra continua em um toque, mas o cadastro tambem precisa ficar acessivel
// para edicao, suporte de acesso e redefinicao de senha.
//
          ...
          {lista.map((cliente) => {
            ...
            return (
              <li key={cliente.id} className={`bal-cliente bal-${situacao}`}>
                <button
                  type="button"
                  className="bal-cliente-texto bal-abrir"
                  onClick={() => void onAbrirCadastro?.(cliente)}
                  disabled={!onAbrirCadastro || ocupado === cliente.id}
                  aria-label={`Abrir cadastro de ${cliente.nome}`}
                >
