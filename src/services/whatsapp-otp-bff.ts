...
const normalizeBrazilPhone = (value: string) => {
  ...
  if (national.length !== 10 && national.length !== 11)
    throw new Error("Informe um WhatsApp com DDD.");
  return `+55${national}`;
};
...
export async function requestAutomaticWhatsAppOtpBff(input: {
  phone: string;
  ...
  const response = await fetch("/api/whatsapp-otp-request", {
    ...
    body: JSON.stringify({
      ...
      idempotencyKey:
        ...
        `whatsapp-otp:${purpose}:${crypto.randomUUID()}`,
    }),
  });
  return readPayload<WhatsAppOtpChallenge>(response);
}

export async function verifyAutomaticWhatsAppOtpBff(
  challengeId: string,
  code: string,
) {
