...
export default function OperationManualSale({ onCreated }: { onCreated: () => void }) {
  ...
  const [channel, setChannel] = useState<ChannelMode>("official");
  ...
  return <>
    <div className="operation-manual-entry-actions">
      <button className="commerce-primary-action" type="button" onClick={() => { setChannel("presential"); setMode("sale"); setNotice(""); setOpen(true); }}><ShoppingBag /> Presencial</button>
      <button className="commerce-secondary-action" type="button" onClick={() => { setChannel("official"); setMode("order"); setNotice(""); setOpen(true); }}><ShoppingBag /> WhatsApp oficial</button>
      <button className="commerce-secondary-action" type="button" onClick={() => { setChannel("private"); setMode("sale"); setNotice(""); setOpen(true); }}><ShoppingBag /> WhatsApp particular</button>
    </div>
    ...
    {open ? <div className="instant-order-operation-layer"><button className="instant-order-operation-backdrop" aria-label="Fechar" onClick={() => setOpen(false)} /><aside role="dialog" aria-modal="true" aria-label={mode === "order" ? "Registrar pedido do WhatsApp" : "Lançar venda"}><button className="drawer-close" onClick={() => setOpen(false)} aria-label="Fechar"><X /></button><div className="manual-sale-channel-switch" role="group" aria-label="Origem do lançamento"><button type="button" className={channel === "presential" ? "active" : ""} onClick={() => { setChannel("presential"); setMode("sale"); }}>Presencial</button><button type="button" className={channel === "official" ? "active" : ""} onClick={() => { setChannel("official"); setMode("order"); }}>WhatsApp oficial</button><button type="button" className={channel === "private" ? "active" : ""} onClick={() => { setChannel("private"); setMode("sale"); }}>WhatsApp particular</button></div><small>{channel === "official" ? "Pedido recebido no WhatsApp oficial" : channel === "private" ? "Pedido recebido no WhatsApp particular" : "Venda presencial"}</small><h2>{mode === "order" ? "Registrar pedido" : "Lançar venda"}</h2><p>{mode === "order" ? "O pedido entra em Para conferir e continua nesta fila até a retirada." : channel === "private" ? "Lance o pedido recebido manualmente. Nenhuma mensagem automática será enviada." : "Use para registrar uma venda presencial e baixar o estoque."}</p>
