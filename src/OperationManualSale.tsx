...
export default function OperationManualSale({ onCreated }: { onCreated: () => void }) {
  ...
  const submit = async () => {
    ...
    if (!openCashSession && (!canReconcileWithoutOpenCash || !selectedStore)) return setNotice("Nenhum caixa está aberto. Abra o caixa antes de baixar esta venda.");
    if (reservationLock.current || busy) return;
