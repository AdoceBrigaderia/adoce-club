...
import CompartilheDocura from "./CompartilheDocura";
import type { Indicacao } from "./compartilhe-docura";
...
export default function AdoceClube() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(async ({ data }) => {
      ...
      const [{ data: perfil }, { data: vinculos }, { data: panorama }] = await Promise.all([
      ...
      const { data: trilhas } = await supabase
        .from("loyalty_tracks").select("id,kind,current_progress,completed_cards").eq("account_id", contaId);

      const principal = trilhas?.find((t) => t.kind === "main");
      const indicacao = trilhas?.find((t) => t.kind === "referral");
      const { count: presentes } = principal
        ? await supabase
            .from("rewards")
            .select("id", { count: "exact", head: true })
            .eq("track_id", principal.id)
            .eq("status", "available")
        : { count: 0 };
      ...
      setEstado({
        ...
        clube: Number(principal?.current_progress || 0) % TOTAL_CLUBE,
        presentes: Number(presentes || 0),
        indicacoes: Number(indicacao?.current_progress || 0) % TOTAL_INDICACAO,
        indicacoesLista: ((panorama?.accepted || []) as Array<{ first_name?: string; status?: string; created_at?: string }>).map((item) => ({
          nome: item.first_name || "Amigo Adoce",
          status: item.status === "confirmed" ? "confirmado" : "convidado",
          quando: item.created_at || null,
        })),
      });
      setCarregando(false);
  ...
  if (!estado) {
    const porta = decidirPorta({ sessao: false, tokenDoCartao: null, membro: null, tokenNaUrl: null });
    return (
  ...
  const porta = decidirPorta({
    sessao: true,
    tokenDoCartao: null,
    membro: { primeiroNome: estado.primeiroNome, carimbos: estado.clube },
    tokenNaUrl: null,
  });

  if (location.hash.startsWith("#indicar")) {
    return <CompartilheDocura cartao={{ codigo: estado.codigo, primeiroNome: estado.primeiroNome, indicacoes: estado.indicacoesLista }} />;
  }
  ...
  return (
    <main className="adoce-clube">
      <header className="ac-topo">
        <div>
          <p className="ac-ola">{porta.titulo}</p>
      ...
      <div className="ac-acoes">
        <a className="ac-principal" href="#adoce-hoje">Pedir fatias de hoje</a>
        <a className="ac-secundario" href="#indicar">Convidar alguém</a>
        <a className="ac-texto" href="#minha-conta">Preferências e QR</a>
      </div>
