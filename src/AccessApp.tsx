...
function CustomerHome({ session }: { session: Session }) {
  ...
  const shareClub=async()=>{
    const text=`Entre para o Clube Adoce com meu código ${snapshot?.referralCode}. Faça seu cadastro e informe este código à equipe antes de confirmar sua primeira compra. Assim, nós dois ganhamos 1 carimbo.`;
...
function OperationHome({ session }: { session: Session }) {
  ...
  const [query,setQuery]=useState(""); const [results,setResults]=useState<CustomerSearchResult[]>([]); const [selected,setSelected]=useState<CustomerSnapshot|null>(null); const [qty,setQty]=useState(1); const [referralCode,setReferralCode]=useState(""); const [message,setMessage]=useState(""); const [busy,setBusy]=useState(false);
  ...
  const purchase=async()=>{if(!selected)return;setBusy(true);setMessage("");const cleanReferral=referralCode.trim().toUpperCase();const {error}=await requireSupabase().rpc("staff_record_purchase",{account_id:selected.account_id,participant_profile_id:selected.profile_id,quantity:qty,idempotency_key:crypto.randomUUID(),referral_code:cleanReferral||null});setBusy(false);if(error)setMessage(error.message);else{setMessage(cleanReferral?`${qty} carimbo(s) da compra registrados. O código ${cleanReferral} foi enviado para validação da primeira compra.`:`${qty} carimbo(s) registrado(s) com sucesso.`);setQty(1);setReferralCode("");await refreshSelected();}};
