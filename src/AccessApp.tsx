...
import { ArrowRight, CakeSlice, Camera, Check, CircleHelp, Download, Gift, Heart, History, LogOut, Mail, Plus, QrCode, Search, ShieldCheck, Smartphone, Sparkles, UserRound, Users, X } from "lucide-react";
...
function InstallGuide({ onClose, onInstall, canInstall }: { onClose?: () => void; onInstall: () => Promise<boolean>; canInstall: boolean }) {
  return <div className="install-guide">
    <div className="install-guide-head"><div><Smartphone/><span><strong>Instale como um aplicativo</strong><small>O Clube fica com ícone próprio na tela do celular.</small></span></div>{onClose&&<button aria-label="Fechar instruções" onClick={onClose}><X/></button>}</div>
    {canInstall&&<button className="access-primary" onClick={()=>void onInstall()}><Download/> Instalar agora</button>}
    <div className="install-platforms"><article><strong>Android · Google Chrome</strong><ol><li>Abra o menu de três pontos.</li><li>Toque em “Instalar app” ou “Adicionar à tela inicial”.</li><li>Confirme em “Instalar”.</li></ol></article><article><strong>iPhone · Safari</strong><ol><li>Toque no botão Compartilhar.</li><li>Escolha “Adicionar à Tela de Início”.</li><li>Confirme em “Adicionar”.</li></ol></article></div>
    <small>O Clube Adoce e o Adoce Operação podem ser instalados separadamente.</small>
  </div>;
}
...
function CustomerHome({ session }: { session: Session }) {
  ...
  const openCustomerQr=async()=>{
    setView("qr"); setBusy(true); setMessage(""); setQrImage("");
    const {data,error:qrError}=await requireSupabase().rpc("issue_customer_qr");
    if(qrError){setBusy(false);setMessage(qrError.message);return;}
    const issued=Array.isArray(data)?data[0]:data;
    if(!issued?.token){setBusy(false);setMessage("Não foi possível gerar seu QR agora.");return;}
    const qrUrl=new URL("/",location.origin);
    qrUrl.searchParams.set("cartao",issued.token);
    qrUrl.hash="cartao";
    const image=await QRCode.toDataURL(qrUrl.toString(),{width:640,margin:2,color:{dark:"#2b130d",light:"#fffaf4"},errorCorrectionLevel:"M"});
    setQrImage(image); setQrExpiresAt(issued.expires_at); setBusy(false);
  };
  ...
  return <main className="club-home"><header><Brand label="Clube Adoce"/><div className="club-header-actions"><a href="/#adoce-hoje"><CakeSlice/> Adoce Hoje</a><button onClick={()=>void openCustomerQr()}><QrCode/> Meu QR</button><button onClick={()=>setView("help")}><CircleHelp/> Como funciona</button><button onClick={()=>void signOut()}><LogOut/> Sair</button></div></header>
    ...
    {view==="qr"&&<section className="club-panel club-qr"><QrCode/><small>Identificação rápida</small><h1>Mostre este QR no atendimento.</h1><p>A equipe lê o código e abre seu cadastro. Nenhuma compra ou resgate acontece sem confirmação na operação.</p>{busy&&!qrImage?<div className="qr-loading">Gerando seu QR seguro...</div>:qrImage&&<><div className="club-qr-image"><img src={qrImage} alt="QR do cartão Clube Adoce"/><img src="/site/logo.webp" alt=""/></div><strong>Válido até {new Date(qrExpiresAt).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</strong><button className="access-secondary" onClick={()=>void openCustomerQr()}><QrCode/> Gerar um novo QR</button></>}</section>}
    {view==="help"&&<section className="club-panel club-help"><CircleHelp/><small>Ajuda do Clube</small><h1>É simples participar.</h1><div className="club-help-list"><article><Heart/><span><strong>Como ganho carimbos?</strong><small>Cada fatia comprada, tradicional ou premium, vale 1 carimbo.</small></span></article><article><QrCode/><span><strong>Como a equipe encontra meu cartão?</strong><small>Abra “Meu QR”, mostre o código no atendimento e a equipe abrirá seu cadastro.</small><button onClick={()=>void openCustomerQr()}>Abrir meu QR</button></span></article><article><Gift/><span><strong>O que acontece ao completar 14?</strong><small>Você ganha uma fatia tradicional ou escolhe uma premium pagando somente a diferença. O prêmio fica guardado até você decidir retirar.</small></span></article><article><Sparkles/><span><strong>Como funciona minha indicação?</strong><small>Compartilhe seu link pessoal. O cadastro fica vinculado automaticamente e, na primeira compra, os dois ganham 1 carimbo.</small><button onClick={()=>setView("share")}>Ver e compartilhar meu link</button></span></article><article><CakeSlice/><span><strong>Onde vejo os sabores disponíveis?</strong><small>Abra o Adoce Hoje para consultar sabores, atendimento e informações atualizadas.</small><a href="/#adoce-hoje">Abrir Adoce Hoje</a></span></article></div><InstallGuide canInstall={canInstall} onInstall={install}/></section>}
    ...
    <nav className="club-bottom"><button className={view==="card"?"active":""} onClick={()=>setView("card")}><Heart/> Cartão</button><button className={view==="qr"?"active":""} onClick={()=>void openCustomerQr()}><QrCode/> Meu QR</button><a href="/#adoce-hoje"><CakeSlice/> Hoje</a><button className={view==="share"?"active":""} onClick={()=>setView("share")}><Users/> Indicar</button><button className={view==="profile"?"active":""} onClick={()=>setView("profile")}><UserRound/> Perfil</button></nav></main>;
}
...
function OperationHome({ session }: { session: Session }) {
  ...
  const [scannerOpen,setScannerOpen]=useState(false); const [scannerStarting,setScannerStarting]=useState(false); const [manualQr,setManualQr]=useState(""); const [installGuideOpen,setInstallGuideOpen]=useState(false);
  const videoRef=useRef<HTMLVideoElement|null>(null); const scannerControls=useRef<IScannerControls|null>(null); const scanHandled=useRef(false);
  const {canInstall,install}=useInstallApp();
  ...
  const stopScanner=useCallback(()=>{scannerControls.current?.stop();scannerControls.current=null;setScannerOpen(false);setScannerStarting(false);},[]);
  const lookupCustomerQr=useCallback(async(qrValue:string)=>{
    const cleanValue=qrValue.trim(); if(!cleanValue){setMessage("Aponte para um QR do Clube Adoce.");return;}
    setBusy(true); setMessage("");
    const {data,error:lookupError}=await requireSupabase().rpc("staff_lookup_customer_by_qr",{qr_value:cleanValue});
    if(lookupError){setBusy(false);setMessage(lookupError.message);return;}
    const customer=(Array.isArray(data)?data[0]:data) as CustomerSearchResult|undefined;
    if(!customer){setBusy(false);setMessage("QR expirado ou cliente não encontrado. Peça ao cliente para gerar um novo código.");return;}
    stopScanner(); setManualQr(""); setBusy(false); await openCustomer(customer);
  },[openCustomer,stopScanner]);
  const startScanner=useCallback(async()=>{
    stopScanner(); setScannerOpen(true); setScannerStarting(true); scanHandled.current=false;
    await new Promise<void>(resolve=>requestAnimationFrame(()=>resolve()));
    try{
      const reader=new BrowserQRCodeReader();
      scannerControls.current=await reader.decodeFromVideoDevice(undefined,videoRef.current!,result=>{
        if(result&&!scanHandled.current){scanHandled.current=true;void lookupCustomerQr(result.getText());}
      });
      setScannerStarting(false);
    }catch(cameraError){setScannerStarting(false);setMessage(cameraError instanceof Error?`Não foi possível abrir a câmera: ${cameraError.message}`:"Não foi possível abrir a câmera.");}
  ...
  return <main className="operation-home"><header><Brand label="Adoce Operação"/><div><span>{role === "owner" ? "Proprietário" : role === "manager" ? "Gerente" : "Atendimento"}</span><button onClick={()=>setInstallGuideOpen(true)}><Download/> Instalar</button><button onClick={()=>void signOut()}><LogOut/> Sair</button></div></header><div className="operation-shell"><aside><button className={view==="attend"?"active":""} onClick={()=>void openView("attend")}><Search/> Atender cliente</button><button className={view==="movements"?"active":""} onClick={()=>void openView("movements")}><History/> Movimentações</button><button className={view==="customers"?"active":""} onClick={()=>void openView("customers")}><Users/> Clientes</button><button className={view==="team"?"active":""} onClick={()=>void openView("team")}><ShieldCheck/> Equipe</button></aside><section className="operation-work">
    ...
    {scannerOpen&&<div className="operation-modal" role="dialog" aria-modal="true" aria-label="Leitor de QR"><div className="scanner-card"><button className="modal-close" onClick={stopScanner} aria-label="Fechar leitor"><X/></button><QrCode/><h2>Leia o QR do cliente</h2><p>Centralize o código na câmera. O cadastro abrirá automaticamente.</p><div className="scanner-video"><video ref={videoRef} muted playsInline/>{scannerStarting&&<span>Abrindo a câmera...</span>}</div><form onSubmit={event=>{event.preventDefault();void lookupCustomerQr(manualQr)}}><label>Ou cole o conteúdo do QR<input value={manualQr} onChange={event=>setManualQr(event.target.value)} placeholder="Link ou código do cartão"/></label><button className="access-secondary" disabled={busy}>Localizar</button></form></div></div>}
    {installGuideOpen&&<div className="operation-modal" role="dialog" aria-modal="true" aria-label="Como instalar o aplicativo"><div className="install-modal"><InstallGuide canInstall={canInstall} onInstall={install} onClose={()=>setInstallGuideOpen(false)}/></div></div>}
