...
do $$
declare
  required_order_tables text[] := array[
    'instant_orders',
    'instant_order_items'
  ];
  missing_tables text[];
...
begin
  ...
  into missing_tables
  from unnest(required_order_tables) table_name
  where to_regclass(format('public.%I', table_name)) is null;

  if coalesce(cardinality(missing_tables), 0) > 0 then
    raise exception 'Instant-order RPC-only tables are missing: %', missing_tables;
  end if;
  ...
  where namespace.nspname = 'public'
    and class.relname = any(required_order_tables)
    and class.relkind in ('r', 'p')
    and not class.relrowsecurity;

  if coalesce(cardinality(rls_disabled), 0) > 0 then
    raise exception 'RLS disabled on instant-order RPC-only tables: %', rls_disabled;
  end if;
  ...
  if coalesce(cardinality(browser_privileges), 0) > 0 then
    raise exception 'Instant-order RPC-only tables expose browser privileges: %',
      browser_privileges;
  end if;
  ...
  where policy.schemaname = 'public'
    and policy.tablename = any(required_order_tables)
    and role_name::text in ('public', 'anon', 'authenticated');

  if coalesce(cardinality(browser_policies), 0) > 0 then
    raise exception 'Instant-order RPC-only tables expose browser policies: %',
      browser_policies;
  end if;
