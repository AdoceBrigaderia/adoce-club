...
declare
  ...
  required_lifecycle_columns text[] := array[
    'payment_status',
    'paid_amount',
  ...
  missing_lifecycle_columns text[];
  workspace_routine regprocedure;
  workspace_definition text;
  lifecycle_routine regprocedure;
  lifecycle_definition text;
  production_routine regprocedure;
...
begin
  ...
  if position('private.staff_has_capability(request.store_id, ''manage_orders'')' in workspace_definition) = 0
     or position('private.staff_has_capability(request.store_id, ''manage_production'')' in workspace_definition) = 0
  ...
  if position('private.staff_has_capability(request.store_id, ''view_finance'')' in workspace_definition) = 0
     or position('else ''Cliente protegido''' in workspace_definition) = 0
     or position('''permissions'', jsonb_build_object' in workspace_definition) = 0 then
  ...
  into missing_lifecycle_columns
  from unnest(required_lifecycle_columns) required_column
  where not exists (
    select 1
    from information_schema.columns column_info
    where column_info.table_schema = 'public'
      and column_info.table_name = 'service_requests'
      and column_info.column_name = required_column
  );

  if coalesce(cardinality(missing_lifecycle_columns), 0) > 0 then
    raise exception 'Service-request lifecycle columns are missing: %', missing_lifecycle_columns;
  end if;

  lifecycle_routine := to_regprocedure(
    'public.staff_transition_service_request(uuid,uuid,text,numeric,text,text)'
  );
  if lifecycle_routine is null then
    raise exception 'staff_transition_service_request lost its protected signature';
  end if;
  if has_function_privilege('anon', lifecycle_routine, 'EXECUTE')
     or not has_function_privilege('authenticated', lifecycle_routine, 'EXECUTE') then
    raise exception 'staff_transition_service_request grants are invalid';
  end if;

  production_routine := to_regprocedure(
    'public.staff_transition_service_request_production(uuid,uuid,text,text)'
  );
  if production_routine is null then
    raise exception 'staff_transition_service_request_production lost its protected signature';
  end if;
  if has_function_privilege('anon', production_routine, 'EXECUTE')
     or not has_function_privilege('authenticated', production_routine, 'EXECUTE') then
    raise exception 'staff_transition_service_request_production grants are invalid';
  end if;
  ...
  select pg_get_functiondef(lifecycle_routine::oid) into lifecycle_definition;
  if position('for update' in lower(lifecycle_definition)) = 0
     or position('pg_advisory_xact_lock' in lifecycle_definition) = 0
     or position('private.staff_has_capability(request_row.store_id, ''manage_orders'')' in lifecycle_definition) = 0
     or position('private.staff_has_capability(request_row.store_id, ''view_finance'')' in lifecycle_definition) = 0
     or position('public.service_request_pricing_snapshots' in lifecycle_definition) = 0 then
    raise exception 'Service-request lifecycle lost locking or capability checks';
  end if;

  select pg_get_functiondef(production_routine::oid) into production_definition;
  if position('for update' in lower(production_definition)) = 0
     or position('private.staff_has_capability(request_row.store_id, ''manage_production'')' in production_definition) = 0 then
    raise exception 'Service-request production lifecycle lost locking or capability checks';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Service-request production guard is missing';
  end if;
end;
$$;
