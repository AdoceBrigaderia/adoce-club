...
create unique index if not exists site_feedback_public_request_key_idx
  on public.site_feedback (public_request_key)
...
begin
  ...
  if requested_category not in ('problem', 'complaint', 'suggestion', 'compliment') then
