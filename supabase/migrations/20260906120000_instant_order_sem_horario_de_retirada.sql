-- Pedido instantaneo: horario de retirada deixa de ser obrigatorio.
--
-- POR QUE.
-- O bot do WhatsApp (e o formulario do site) obrigavam o cliente a escolher um
-- horario fixo de retirada entre 20:00 e 23:00 -- heranca da antiga feira do
-- "Cantinho da Adoce". Nao existe mais essa regra: o cliente nao precisa dizer
-- que horas vai buscar nem quem vai buscar. A loja so informa que a entrega da
-- Adoce e para pedidos de 5 fatias ou mais; abaixo disso o cliente retira no
-- local ou manda um entregador de aplicativo coletar, sempre depois que a loja
-- avisa que o pedido esta separado.
--
-- O QUE MUDA em submit_instant_order_v7:
--   * requested_pickup_time passa a ser OPCIONAL (pode ser null)
--   * a checagem "nao pode estar no passado" so roda se um horario foi informado
--   * requested_pickup_method aceita null (alem de 'customer'/'driver')
--   * a checagem de fornada por horario, quando nao ha horario, usa o fim do
--     dia como referencia (ou seja, vale a disponibilidade total do dia)
--
-- server_submit_whatsapp_order ganha default null nos dois ultimos parametros
-- para o bot poder chamar sem eles.
--
-- Nada de schema, RLS, caldas, clube, pagamento, lotes ou idempotencia muda.
...
declare
  ...
  batch_deadline time := coalesce(requested_pickup_time, time '23:59:59');
begin
  ...
  -- Horario de retirada e OPCIONAL. Se vier, nao pode estar no passado.
  if requested_pickup_method is not null
     and requested_pickup_method not in ('customer', 'driver') then
    raise exception 'Método de retirada inválido';
  end if;
  if requested_pickup_time is not null and requested_pickup_time < local_time then
    raise exception 'Escolha um horário de retirada que ainda não passou';
  end if;

  -- A checagem contra business_hours ficava aqui. Ver 20260819170000.
  ...
  loop
    ...
    if private.flavor_batch_quantity_by_time(
      ...
      raise exception 'A quantidade escolhida não está disponível para hoje';
    end if;
  end loop;
  if requested_reward is not null and requested_reward <> 'null'::jsonb
    ...
    raise exception 'A fatia-presente escolhida não está disponível para hoje';
  end if;
...
-- Bot passa a chamar sem os dois ultimos argumentos.
create or replace function public.server_submit_whatsapp_order(
  ...
  requested_pickup_time time without time zone default null,
  requested_pickup_method text default null
)
returns jsonb
language sql
security definer
