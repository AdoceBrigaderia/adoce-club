...
-- A versão original do workspace validava apenas se o operador possuía a
-- capacidade em alguma loja. Como as encomendas agora possuem store_id
-- obrigatório, a leitura de dados pessoais e financeiros também deve ser
-- limitada às lojas efetivamente autorizadas para a sessão.
...
comment on function public.staff_get_service_request_workspace(text,text,integer,uuid) is
  'Lista encomendas somente das lojas autorizadas, com filtro explícito de unidade e custos condicionados à capacidade financeira.';

-- Pós-gate fail-closed: impede a permanência da assinatura antiga ou a perda
-- dos predicados obrigatórios de loja/capacidade.
do $$
declare
  new_routine regprocedure := to_regprocedure(
...
begin
  if to_regprocedure('public.staff_get_service_request_workspace(text,text,integer)') is not null then
    raise exception 'A assinatura antiga do workspace de encomendas permanece disponível';
  end if;

  if new_routine is null then
    raise exception 'A assinatura protegida do workspace de encomendas não foi criada';
  end if;

  if has_function_privilege('anon', new_routine, 'EXECUTE') then
    raise exception 'O workspace de encomendas permanece executável por anon';
  end if;

  if not has_function_privilege('authenticated', new_routine, 'EXECUTE') then
    raise exception 'O BFF autenticado perdeu acesso ao workspace de encomendas';
  end if;

  select pg_get_functiondef(new_routine::oid) into definition;
  ...
  if position('private.staff_has_capability(request.store_id, ''manage_orders'')' in definition) = 0
     ...
     or position('private.staff_has_capability(request.store_id, ''view_finance'')' in definition) = 0 then
    raise exception 'O workspace de encomendas perdeu isolamento por loja ou teto financeiro';
  end if;
end;
$$;
