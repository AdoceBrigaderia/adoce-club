...
set subject_profile_id = membership.profile_id
from public.loyalty_tracks as track
join public.account_memberships as membership
  on membership.account_id = track.account_id
 and membership.active
 and membership.is_primary
where entry.track_id = track.id
  and entry.subject_profile_id is null;
...
begin
  ...
  select membership.profile_id into customer_profile_id
  from public.loyalty_tracks as track
  join public.account_memberships as membership
    on membership.account_id = track.account_id
   and membership.active
  where track.id = reward_row.track_id
  order by membership.is_primary desc, membership.joined_at asc
  limit 1;
