-- Sincronização segura do catálogo comercial com a Meta.
...
begin
  ...
  if new.meta_retailer_id is distinct from old.meta_retailer_id then
    raise exception 'O identificador permanente da Meta não pode ser alterado';
  end if;
...
comment on column public.commercial_products.meta_retailer_id is
  'Identificador permanente usado como product_retailer_id na Meta; não muda com nome ou descrição.';
comment on table public.meta_catalog_sync_history is
  'Trilha administrativa da integração com a Meta. Sem acesso público e sem credenciais.';
